...
$getOrders = $koneksi->query("SELECT * FROM orders WHERE status = 'pending' ORDER BY id_order DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <?php if($getOrders->num_rows > 0): ?>
            <?php $ordinal = 0; foreach($getOrders as $order): $ordinal++; $bukti_tipe_file = "image/jpeg"; $bukti_data = base64_encode($order['bukti']); ?>
                ...
                <tr>
                    ...
                    <td>Rp. <?= number_format($db->getDataVenues($order['id_venue'])['tarif']) ?></td>
                    ...
                    <td>
                        <button data-modal-target="verifikasiModal<?= $ordinal ?>" data-modal-toggle="verifikasiModal<?= $ordinal ?>" class="transition cursor-pointer w-full rounded-lg bg-blue-700 px-3 py-2 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 lg:w-auto">Verifikasi</button>
