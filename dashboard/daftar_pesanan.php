...
$getOrders = $koneksi->query("SELECT * FROM orders ORDER BY id_order DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <?php if($getOrders->num_rows > 0): ?>
            <?php $ordinal = 0; foreach($getOrders as $order): $ordinal++; $status = $order['status']; $bukti_tipe_file = "image/jpeg"; $bukti_data = base64_encode($order['bukti']); ?>
                ...
                <tr>
                    ...
                    <td><?= $db->getDataVenues($order['id_venue'])['venue'] ?></td>
                    ...
                    <td>
                        ...
                        <?php if($status == "pending"): ?>
                            <button data-modal-target="confirmModal<?= $ordinal ?>" data-modal-toggle="confirmModal<?= $ordinal ?>" class="transition w-full rounded-lg border border-red-700 px-3 py-2 text-center text-sm font-medium text-red-700 hover:bg-red-700 hover:text-white focus:outline-none  dark:border-red-500 dark:text-red-500 dark:hover:bg-red-600 dark:hover:text-white lg:w-auto">Hapus</button>
                        ...
                        <?php elseif($status == "active"): ?>
                            <button data-modal-target="confirmModal<?= $ordinal ?>" data-modal-toggle="confirmModal<?= $ordinal ?>" class="transition w-full rounded-lg border border-red-700 px-3 py-2 text-center text-sm font-medium text-red-700 hover:bg-red-700 hover:text-white focus:outline-none  dark:border-red-500 dark:text-red-500 dark:hover:bg-red-600 dark:hover:text-white lg:w-auto">Batalkan</button>
                                    ...
                                    <dl>
                                        <div class="text-sm space-y-4 sm:space-y-2 rounded-lg border border-gray-100 bg-gray-50 p-5 dark:border-gray-700 dark:bg-gray-800 mb-6 md:mb-8">
                                            ...
                                            <dl class="sm:flex items-center justify-between gap-4">
                                                ...
                                                <dd class="font-medium text-gray-900 dark:text-white sm:text-end"><?= $db->getDataUser($order['id_user'])['email'] ?></dd>
                                            </dl>
                                            <dl class="sm:flex items-center justify-between gap-4">
                                                <dt class="font-normal sm:mb-0 text-gray-500 dark:text-gray-400">No. Telepon Akun</dt>
                                                <dd class="font-medium text-gray-900 dark:text-white sm:text-end"><?= $db->getDataUser($order['id_user'])['telephone'] ?></dd>
                                            </dl>
