                ...
                <?php $ordinal = 0; foreach($getOrders as $order): $ordinal++; $status = $order['status']; $bukti_tipe_file = "image/jpeg"; $bukti_data = base64_encode($order['bukti']); ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if($status == "pending" || $status == "expired" || $status == "cancel"): ?>
                                <button data-modal-target="confirmModal<?= $ordinal ?>" data-modal-toggle="confirmModal<?= $ordinal ?>" class="transition w-full rounded-lg px-3 py-2 text-center text-sm font-medium text-white bg-red-600 hover:bg-red-700  :bg-red-600 :text-white lg:w-auto"><i class="fa-regular fa-trash-can w-5"></i></button>
                                ...
                                <div id="confirmModal<?= $ordinal ?>" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                                        <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
                                            <button type="button" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center :bg-gray-600 :text-white" data-modal-toggle="confirmModal<?= $ordinal ?>">
                            ...
                            <?php elseif($status == "active"): ?>
                                <button data-modal-target="confirmModal<?= $ordinal ?>" data-modal-toggle="confirmModal<?= $ordinal ?>" class="transition w-full rounded-lg px-3 py-2 text-center text-sm font-medium text-white bg-red-600 hover:bg-red-700  :bg-red-600 :text-white lg:w-auto"><i class="fa-regular fa-flag w-5"></i></button>
