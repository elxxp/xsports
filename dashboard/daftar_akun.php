            ...
            <tbody>
                <?php if($getUsers->num_rows > 0): ?>
                    <?php $ordinal = 0; foreach($getUsers as $user): $ordinal++; ?>
                        ...
                        <tr>
                            <td class="font-medium text-gray-900 whitespace-nowrap dark:text-white">#<?= $user['id_user'] ?></td>
                            <td><?= $user['name'] ?></td>
                            <td><?= $user['email'] ?></td>
                            ...
                            <td>
                                <form action="../core/form/updLevelUser" method="post">
                                    ...
                                    <select onchange="this.form.submit()" name="level" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            ...
                            <td>
                                <button data-modal-target="confirmModalDelete" data-modal-toggle="confirmModalDelete" class="transition w-full rounded-lg px-3 py-2 text-center text-sm font-medium text-white bg-red-600 hover:bg-red-700 dark:text-red-500 dark:hover:bg-red-600 dark:hover:text-white lg:w-auto"><i class="fa-regular fa-trash-can w-4"></i></button>
                            
                                <!-- confirm modal -->
                                <div id="confirmModalDelete" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                                        <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                                            <button type="button" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="confirmModalDelete">
                                                <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                                                <span class="sr-only">Close modal</span>
                                            </button>

                                            <p class="mb-2 mt-1 font-semibold text-gray-500 dark:text-gray-300"><i class="fa-regular fa-triangle-exclamation mx-2"></i>Hapus akun <span class="font-bold">#<?= $user['id_user'] ?></span> ?</p>
                                            <p class="mb-5 ml-2 text-sm text-gray-500 dark:text-gray-300">Akun yang dihapus tidak dapat dikembalikan kembali, dan data seluruh pesanan yang terkait dengan akun #<?= $user['id_user'] ?> juga akan terhapus. Yakin menghapus?</p>
                                            <div class="flex justify-end items-center space-x-2">
                                                <form action="../core/form/deleteAccount" method="post">
                                                    <input type="hidden" name="account" value="<?= $user['id_user'] ?>">
                                                    <button data-modal-toggle="confirmModalDelete" type="button" class="py-2 px-3 text-sm font-medium text-gray-500 bg-white rounded-lg border border-gray-200 hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-slate-300 hover:text-gray-900 focus:z-10 dark:bg-gray-700 dark:text-gray-300 dark:border-gray-500 dark:hover:text-white dark:hover:bg-gray-600 dark:focus:ring-gray-600">
                                                        kembali
                                                    </button>
                            ...
                            <section>
                                <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
                                    <div class="mx-auto max-w-screen-sm text-center">
                                        <h1 class="mb-4 text-7xl tracking-tight font-extrabold lg:text-9xl text-blue-600 dark:text-blue-500"><i class="fa-regular fa-user-secret"></i></h1>
                                        <p class="mb-4 text-3xl tracking-tight font-bold text-gray-900 md:text-4xl dark:text-white">Tidak ada pengguna selain anda</p>
                                    </div>   
                                </div>
                            </section>
                        </td>
