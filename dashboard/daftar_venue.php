...
$getOrders = $koneksi->query("SELECT * FROM venues ORDER BY id_venue DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
        ...
        <?php if($getOrders->num_rows > 0): ?>
            <?php $ordinal = 0; foreach($getOrders as $order): $ordinal++; $status = $order['status']; $bukti_tipe_file = "image/jpeg"; @$bukti_data = base64_encode($order['thumbnail']); ?>
                ...
                <tr>
                    ...
                    <td>
                        <?php if($status == "open"): ?>
                            <button data-modal-target="confirmModal<?= $ordinal ?>" data-modal-toggle="confirmModal<?= $ordinal ?>" class="transition cursor-pointer w-full inline-flex justify-center rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700 lg:w-auto">Close venue</button>
                            ...
                            <?php elseif($status == "closed"): ?>
                            <button data-modal-target="confirmModal<?= $ordinal ?>" data-modal-toggle="confirmModal<?= $ordinal ?>" class="transition cursor-pointer w-full inline-flex justify-center rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:hover:text-white dark:focus:ring-gray-700 lg:w-auto">Buka venue</button>
