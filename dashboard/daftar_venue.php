        ...
        <tbody>
            <?php if($getOrders->num_rows > 0): ?>
                <?php $ordinal = 0; foreach($getOrders as $order): $ordinal++; $status = $order['status']; $bukti_tipe_file = "image/jpeg"; @$bukti_data = base64_encode($order['thumbnail']); ?>
                    ...
                    <tr>
                        <td class="font-medium text-gray-900 whitespace-nowrap ">#<?= $order['id_venue'] ?></td>
                        <td><?= $order['venue'] ?></td>
                        <td class="font-medium text-gray-900 whitespace-nowrap "><?= $order['sport'] ?></td>
                        <td class="font-bold text-gray-900 whitespace-nowrap ">Rp. <?= number_format($order['tarif']) ?></td>
                        <td>
                            <button data-modal-target="cardOvModal<?= $ordinal ?>" data-modal-toggle="cardOvModal<?= $ordinal ?>" class="transition cursor-pointer w-full inline-flex justify-center rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100    :bg-gray-700 :text-white :ring-gray-700 lg:w-auto">View card</button>
                                ...
                                <div class="relative p-4 w-full max-w-xl h-full md:h-auto">
                                    <div class="relative bg-white rounded-lg shadow  sm:p-6">
                                            ...
                                            <div class="text-sm space-y-4 sm:space-y-2 rounded-lg border border-gray-100 bg-gray-50 p-5   mb-6 md:mb-8">
                                                <dl>
                                                    <!-- card -->
                                                    <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow-sm   cursor-default">
                                                        <div>
                                                        ...
                                                        <div class="p-5">
                                                            <div>
                                                                <h5 class="mb-1 text-2xl font-bold tracking-tight text-gray-900 "><?= $order['venue'] ?></h5>
                                                            ...
                                                            <h5 class="mt-3 tracking-tight text-gray-900 "><span class="text-xl font-bold text-emerald-600 ">Rp. <?= number_format($order['tarif']) ?></span> / jam</h5>
                                                            <p class="font-normal text-gray-700 "><?= $order['description'] ?></p>
                                                            <div class="flex items-center gap-2 mt-5">
                                                                <?php if($order['status'] == 'open'): ?>
                                                                    <button class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-1.5  :bg-blue-700 focus:outline-none :ring-blue-800 cursor-pointer">
                                                                        Booking sekarang
                                                                    </button>
                                                                <?php else: ?>
                                                                    <button class="text-white bg-blue-700 font-medium rounded-lg text-sm px-4 py-1.5  opacity-40" disabled>
                                                                        Booking sekarang
                                                                    </button>
                                                                <?php endif; ?>
                                                            </div>
                                        ...
                                        <div>   
                                            <button type="button" data-modal-toggle="cardOvModal<?= $ordinal ?>" class="cursor-pointer py-2.5 w-full px-5 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 focus:z-10 focus:ring-4 focus:ring-gray-200 :ring-gray-700   ">
                                                Tutup
                                            </button>             
                                        </div>
                                    </div>
                        ...
                        <td>
                            <?php if($status == "open"): ?>
                                ...
                                <div id="confirmModalSet<?= $ordinal ?>" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-modal md:h-full">
                                    <div class="relative p-4 w-full max-w-md h-full md:h-auto">
                                        <div class="relative p-4 bg-white rounded-lg shadow  sm:p-5">
                                            <button type="button" class="text-gray-400 absolute top-2.5 right-2.5 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center :bg-gray-600 :text-white" data-modal-toggle="confirmModalSet<?= $ordinal ?>">
                                ...
                                <?php elseif($status == "closed"): ?>
                                <button data-modal-target="confirmModalSet<?= $ordinal ?>" data-modal-toggle="confirmModalSet<?= $ordinal ?>" class="transition cursor-pointer w-25 inline-flex justify-center rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-900 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100    :bg-gray-700 :text-white :ring-gray-700">Set open</button>
                            ...
                            <button onclick="location.href='update_venue?id_venue=<?= $order['id_venue'] ?>'" class="transition cursor-pointer w-full rounded-lg bg-blue-700 px-3 py-2 text-sm font-medium text-white hover:bg-blue-800  :bg-blue-700 lg:w-auto"><i class="fa-regular fa-pen w-4"></i></button>
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
                        ...
                        <section>
                            <div class="py-8 px-4 mx-auto max-w-screen-xl lg:py-16 lg:px-6">
                                <div class="mx-auto max-w-screen-sm text-center">
                                    <h1 class="mb-4 text-7xl tracking-tight font-extrabold lg:text-9xl text-blue-600 "><i class="fa-regular fa-volleyball"></i></h1>
                                    <p class="mb-4 text-3xl tracking-tight font-bold text-gray-900 md:text-4xl ">Tidak memiliki venue</p>
                                </div>   
                            </div>
                        </section>
                    </td>
